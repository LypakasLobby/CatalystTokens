...
import com.lypaka.bettertokens.API.GivePointsEvent;
import com.lypaka.bettertokens.API.RemovePointsEvent;
...
public class ConfigGetters {
    ...
    public static void load() throws ObjectMappingException {
        ...
        message = BetterTokens.configManager.getConfigNode(0,"Message").getString();
        useBetterGUIs = BetterTokens.configManager.getConfigNode(0, "BetterGUIs-Installed").getBoolean();
        mainGUIName = BetterTokens.configManager.getConfigNode(0, "GUI", "Name").getString();
        mainGUIRows = BetterTokens.configManager.getConfigNode(0, "GUI", "Rows").getInt();
        mainGUIBorderID = BetterTokens.configManager.getConfigNode(0, "GUI", "Slots", "Border", "ID").getString();
        mainGUIBorderMeta = 0;
        if (!BetterTokens.configManager.getConfigNode(0, "GUI", "Slots", "Border", "Meta").isVirtual()) {

            mainGUIBorderMeta = BetterTokens.configManager.getConfigNode(0, "GUI", "Slots", "Border", "Meta").getInt();

        }
        mainGUIBorderSlots = BetterTokens.configManager.getConfigNode(0, "GUI", "Slots", "Border", "Slots").getString();
        mainGUIMenuSlots = BetterTokens.configManager.getConfigNode(0, "GUI", "Slots", "Menus").getValue(new TypeToken<Map<String, Map<String, String>>>() {});
        menuList = BetterTokens.configManager.getConfigNode(0, "Menus").getList(TypeToken.of(String.class));

        allowEvents = BetterTokens.configManager.getConfigNode(1, "Allow-Events").getBoolean();
        eventsMap = BetterTokens.configManager.getConfigNode(1, "Events").getValue(new TypeToken<Map<String, Map<String, String>>>() {});

        pointsMap = BetterTokens.configManager.getConfigNode(2, "Points").getValue(new TypeToken<Map<String, Double>>() {});
